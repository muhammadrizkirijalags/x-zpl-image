...
class ImageArrayBuffer extends Uint8Array {
    /**
    ...
    constructor(length: number, width: number = 0, height: number = 0) {
        super(length);
        this.width = width;
        this.height = height;
    }
...
export function imageToZ64(img: HTMLImageElement, opts: RGBAToZ64Options) {
...
// DOM-specialized version for browsers.
export function imageToACS(img: HTMLImageElement, opts: RGBAToZ64Options) {
	// Draw the image to a temp canvas so we can access its RGBA data
	let cvs = document.createElement('canvas');
	let ctx = cvs.getContext('2d')!;
...
type RGBAToZ64Options = {
    rotate: 'R' | 'L' | 'I' | 'B' | 'N' | string
    black: number,
    notrim: boolean,
}
...
//			'B' same as 'L'
export function rgbaToZ64(rgba: ImageDataArray, width: number, opts: RGBAToZ64Options) {
	opts = opts || {};
	width = width|0;
	if (!width || width < 0) {
	...
	let imgh = buf.height;
	let rowl = ~~((imgw + 7) / 8);
	let b64 = u8tob64(pako.deflate(buf));
...
//			'I' rotate 180 degrees (inverted)
//			'B' same as 'L'
export function rgbaToACS(rgba: ImageDataArray, width: number, opts: RGBAToZ64Options) {
	opts = opts || {};
	width = width|0;
	if (!width || width < 0) {
		throw new Error('Invalid width');
...
function normal(mono: ImageArrayBuffer) {
	let width	= mono.width;
	let height	= mono.height;

	let buf	 = new ImageArrayBuffer(~~((width + 7) / 8) * height);
...
function invert(mono: ImageArrayBuffer) {
	let width	= mono.width;
	let height	= mono.height;
...
function left(mono: ImageArrayBuffer) {
	let width	= mono.width;
	let height	= mono.height;

	let buf	 = new ImageArrayBuffer(~~((height + 7) / 8) * width);
	let idx	 = 0;	// index into buf
	let byte = 0;	// current byte of image data
...
function right(mono: ImageArrayBuffer) {
	let width	= mono.width;
	let height	= mono.height;
...
function monochrome(rgba: ImageDataArray, width: number, height: number, black: number, notrim: boolean): ImageArrayBuffer {
	...
	let cx = maxx - minx + 1;
	let cy = maxy - miny + 1;
	let buf = new ImageArrayBuffer(cx * cy, cx, cy);
	let idx = 0;
...
/**
 * Cyclic Redundancy Check checksum for a string
 * @param s 
 * @returns 
 */
function crc16(s: string) {
	...
	const strcrc = (crc & 0xffff).toString(16).toLowerCase();
	return '0000'.substr(strcrc.length) + strcrc;
}
